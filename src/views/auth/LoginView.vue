<template>
    <div>
        <section class="auth d-flex align-items-center position-relative">
            <div class="container-fluid">
                <div class="row justify-content-center align-items-center">
                    <!-- AUTH IMAGE -->
                    <div class="col-xl-6 col-lg-6">
                        <div class="image_section  d-none d-lg-block">
                            <h2>Sign In to <span class="pink-text">Hideous</span></h2>
                            <div class="main-img">
                                <img src="@/assets/images/auth/main-img.png" class="img-fluid">
                            </div>
                        </div>
                    </div>
                    <!-- AUTH FORM -->
                    <div class="col-xl-4 col-lg-6 col-md-9">
                        <div class="form_box  px-4 py-5 px-md-5">
                            <form @submit.prevent="submit">
                                <!-- EMAIL -->
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email </label>
                                    <input type="email" class="form-control shadow-none border-0" id="email"
                                        placeholder="Enter Email id" v-model="v$.email.$model">
                                    <div class="input-errors" v-for="error of v$.email.$errors" :key="error.$uid">
                                        <div class="text-danger">{{ error.$message }}</div>
                                    </div>
                                </div>
                                <!-- PASSWORD -->
                                <div class="mb-3 password">
                                    <label for="password" class="form-label">Password</label>
                                    <input :type="type" class="form-control shadow-none border-0" id="password"
                                        placeholder="Password" v-model="v$.password.$model">
                                    <span class="float-end position-relative eye" v-if="type=='password'"
                                        @click="type='text'">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" height="18"
                                            width="18" style="fill:#2b2b2ba8">
                                            <path
                                                d="M279.6 160.4C282.4 160.1 285.2 160 288 160C341 160 384 202.1 384 256C384 309 341 352 288 352C234.1 352 192 309 192 256C192 253.2 192.1 250.4 192.4 247.6C201.7 252.1 212.5 256 224 256C259.3 256 288 227.3 288 192C288 180.5 284.1 169.7 279.6 160.4zM480.6 112.6C527.4 156 558.7 207.1 573.5 243.7C576.8 251.6 576.8 260.4 573.5 268.3C558.7 304 527.4 355.1 480.6 399.4C433.5 443.2 368.8 480 288 480C207.2 480 142.5 443.2 95.42 399.4C48.62 355.1 17.34 304 2.461 268.3C-.8205 260.4-.8205 251.6 2.461 243.7C17.34 207.1 48.62 156 95.42 112.6C142.5 68.84 207.2 32 288 32C368.8 32 433.5 68.84 480.6 112.6V112.6zM288 112C208.5 112 144 176.5 144 256C144 335.5 208.5 400 288 400C367.5 400 432 335.5 432 256C432 176.5 367.5 112 288 112z" />
                                        </svg>
                                    </span>
                                    <span class="float-end position-relative eye" v-else @click="type='password'">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" height="18"
                                            width="18" style="fill:#2b2b2ba8">
                                            <path
                                                d="M150.7 92.77C195 58.27 251.8 32 320 32C400.8 32 465.5 68.84 512.6 112.6C559.4 156 590.7 207.1 605.5 243.7C608.8 251.6 608.8 260.4 605.5 268.3C592.1 300.6 565.2 346.1 525.6 386.7L630.8 469.1C641.2 477.3 643.1 492.4 634.9 502.8C626.7 513.2 611.6 515.1 601.2 506.9L9.196 42.89C-1.236 34.71-3.065 19.63 5.112 9.196C13.29-1.236 28.37-3.065 38.81 5.112L150.7 92.77zM223.1 149.5L313.4 220.3C317.6 211.8 320 202.2 320 191.1C320 180.5 316.1 169.7 311.6 160.4C314.4 160.1 317.2 159.1 320 159.1C373 159.1 416 202.1 416 255.1C416 269.7 413.1 282.7 407.1 294.5L446.6 324.7C457.7 304.3 464 280.9 464 255.1C464 176.5 399.5 111.1 320 111.1C282.7 111.1 248.6 126.2 223.1 149.5zM320 480C239.2 480 174.5 443.2 127.4 399.4C80.62 355.1 49.34 304 34.46 268.3C31.18 260.4 31.18 251.6 34.46 243.7C44 220.8 60.29 191.2 83.09 161.5L177.4 235.8C176.5 242.4 176 249.1 176 255.1C176 335.5 240.5 400 320 400C338.7 400 356.6 396.4 373 389.9L446.2 447.5C409.9 467.1 367.8 480 320 480H320z" />
                                        </svg>
                                    </span>

                                    <div class="input-errors" v-for="error of v$.password.$errors" :key="error.$uid">
                                        <div class="text-danger">{{ error.$message }}</div>
                                    </div>
                                </div>
                                <!-- FORGOT PASSWORD -->
                                <div class="mb-3 text-end">
                                    <p class="pe-3">
                                        <router-link to="/forgot-password" class="forgot-pass">Forgot Password ?
                                        </router-link>
                                    </p>
                                </div>

                                <!--Recaptcha feild-->
                                <div>
                                    <vue-recaptcha ref="recaptcha" :sitekey="recaptchaKey"
                                        v-model="v$.recaptchaModel.$model"
                                        @verify="(capRes)=>{ state.recaptchaModel = capRes }" />
                                    <!--@verify="(capRes)=>{ state.recaptchaModel = capRes }"-->
                                    <div class="input-errors" v-for="error of v$.recaptchaModel.$errors"
                                        :key="error.$uid">
                                        <div class="text-danger">{{ error.$message }}</div>
                                    </div>
                                </div>
                                <!-- BUTTON -->
                                <div class="d-grid mb-3">
                                    <button type="submit" class="btn auth_btn text-white shadow-none"
                                        v-if="!loader">Sign In</button>
                                    <button type="button" class="btn auth_btn text-white shadow-none" disabled v-else>
                                        <div class="d-flex justify-content-center">
                                            <div class="spinner-border" role="status"
                                                style="width: 1rem; height: 1rem;">
                                                <span class="visually-hidden">Loading...</span>
                                            </div>
                                        </div>
                                    </button>


                                </div>
                                <!-- AUTH OPTION -->
                                <div class="text-center options">
                                    <p>If you don't have an account,
                                        <span>
                                            <router-link to="/register">Sign Up</router-link>
                                        </span>
                                    </p>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
</template>

<script>
import { reactive, ref } from 'vue'
import useVuelidate from '@vuelidate/core'
import { required, email, helpers } from '@vuelidate/validators'
import ApiClass from '@/api/api'
import { VueRecaptcha } from 'vue-recaptcha';
export default {
    name: 'LoginView',
    components: { VueRecaptcha },
    setup() {
        const type = ref('password')
        const loader = ref(false)
        const recaptchaKey = ref(ApiClass.CAPTCHA_SITE_KEY)
        const state = reactive({
            email: '',
            password: '', 
            recaptchaModel:''        
        })
        let form = reactive(state)
        const rules = {
            email: { required: helpers.withMessage('Email feild is required',required), email: helpers.withMessage('Value must be an email',email) },
            password: { required: helpers.withMessage('Password feild is required', required) },
            recaptchaModel: { required: helpers.withMessage('Recaptcha feild is required', required) }      
        }

        const v$ = useVuelidate(rules, state)
        
        return { state, type, v$, recaptchaKey, loader, form }
    },
    methods: {
        async submit() {
           
            const result = await this.v$.$validate()
            if (!result) {
                return
            }
            this.loader = true;
            let res = await ApiClass.postRequest('login', false, { email: this.state.email, password: this.state.password, captcha_response: this.state.recaptchaModel  });
            if (res?.data)
            {
                this.loader = false;

                if (res.data?.status_code == 0) {
                    this.$refs.recaptcha.reset();
                    this.state.recaptchaModel = '';
                    return this.failed(res.data.message);
                }

                this.success(res.data.message);
                this.reSet();
                if (res.data.data.token) { 
                    localStorage.setItem('token', res.data.data.token)
                    localStorage.setItem('user', JSON.stringify(res.data.data.user))
                    await new Promise((resolve) => setTimeout(resolve, 1000)); 
                    this.$router.push("/exchange");                    
                }
                else {                    
                    this.$router.push({
                        name: 'VerificationView',
                        params:
                        {
                        optData : JSON.stringify(res.data.data)
                    }});                 

                }
            }            
        },
        reSet() {
            this.v$.$reset();
            this.form = reactive(this.state);
        }

    }
}
</script>
